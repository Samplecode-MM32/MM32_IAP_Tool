...
* @retval : None
*/
void UART_ClearFlag(UART_TypeDef* UARTx, uint16_t UART_FLAG)	//??chend: the CSR Reg is R(ead)?
{
    /* Check the parameters */
    assert_param(IS_UART_ALL_PERIPH(UARTx));
    assert_param(IS_UART_CLEAR_FLAG(UART_FLAG));
    
	//  UARTx->CSR = (uint16_t)~UART_FLAG; //??chend: replace logic
...
void UART_SetRXAddress(UART_TypeDef* UARTx, uint8_t UART_Address)
{
    ...
    UARTx->RXADDR &= RX_Address_Mask;
    /* Set the UART address node */
    UARTx->RXADDR |= UART_Address;
}
...
void UART_SendBreak(UART_TypeDef* UARTx)
{
  ...
  UARTx->CCR |= CCR_BRK_Set;        //cd?? when test, something error -> ISR;
}
/**
* @}
*/
