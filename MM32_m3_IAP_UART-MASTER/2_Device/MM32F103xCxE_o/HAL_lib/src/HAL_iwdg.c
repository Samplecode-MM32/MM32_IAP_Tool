...
/**
* @brief  Enables or disables the specified IWDG interrupts.
* @param NewState: new state of the IWDG General call.
...
{       
    if (NewState != DISABLE)
    {
        /* Enable the selected IWDG interrupts */
        IWDG->CTRL |= IWDG_IT_IRQ_SEL;
    }
    else
    {
        /* Disable the selected IWDG interrupts */
        IWDG->CTRL &= ~IWDG_IT_IRQ_SEL;
    }
}

/**
* @brief  Clears the IWDG interrupt pending bits.
* @param NewState: new state of the IWDG General call.
*   This parameter can be: ENABLE or DISABLE.			                             
* @retval : None
*/
void IWDG_ClearITPendingBit(FunctionalState NewState)
{
	if (NewState != DISABLE)
    {
        /* Enable the selected IWDG interrupts */
        IWDG->CTRL |= IWDG_IT_IRQ_CLR;
    }
    else
    {
        /* Disable the selected IWDG interrupts */
