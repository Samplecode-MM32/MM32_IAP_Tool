...
#ifndef USE_IAR
const uint8_t ConstData[1] __attribute__((at(0x8002C00))) = {0x55} ; 
#else
const uint8_t ConstData[13*1011]                                               \
    __attribute__((section(".ARM.__at_0x8002C00"))) = {0x55} ;
#endif

#ifndef ASCII_EN
static uint8_t NeedProtect = 1 ;  /* Is read protection required */
...
static void Iap_Jump_To_Address(uint32_t wUserFlashAddr)
{
    if (((*(__IO uint32_t *)(wUserFlashAddr + 4)) & 0xFF000000) == 0x08000000)
    {
        ...
        printf("Application error, unable to execute!\r\n");
        #endif
    }
    else
    {
        #ifdef DEBUG_EN
        printf("Not a flash application, unable to execute!                   \
        \r\n");
        #endif
    ...
    while (1)            
    {
        ...
        {  /* Test data is :  55 01 00 4B 01 88 99 AA BB CC FF */
            ...
            {                               
                ...
                Iap_Jump_To_Address(FLASH_BASE_ADDR) ; /*  */            
            }
        }
